...
selectTag = """
select distinct(tag1) as tag from deal_history where tag1 is not null and site not in ('ppomppu.co.kr', 'clien.net') union all
select distinct(tag2) as tag from deal_history where tag2 is not null and site not in ('ppomppu.co.kr', 'clien.net') union all
select distinct(tag3) as tag from deal_history where tag3 is not null and site not in ('ppomppu.co.kr', 'clien.net') union all
select distinct(tag4) as tag from deal_history where tag4 is not null and site not in ('ppomppu.co.kr', 'clien.net') union all
select distinct(tag5) as tag from deal_history where tag5 is not null and site not in ('ppomppu.co.kr', 'clien.net') union all
select distinct(tag6) as tag from deal_history where tag6 is not null and site not in ('ppomppu.co.kr', 'clien.net') union all
select distinct(tag7) as tag from deal_history where tag7 is not null and site not in ('ppomppu.co.kr', 'clien.net') union all
select distinct(tag8) as tag from deal_history where tag8 is not null and site not in ('ppomppu.co.kr', 'clien.net') 
"""
...
try:
    ...
    with open('tag_dictionary.pkl', 'wb') as output:
    	pickle.dump(tagKeyword, output, pickle.HIGHEST_PROTOCOL)

    with open('tag_dictionary.pkl', 'rb') as input:
